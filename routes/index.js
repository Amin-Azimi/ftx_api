...
router.get('/live/:id' ,function(req, res, next) {
  ...
  if(req.params.id == 1){
    var lv_url = "http://iptv.tivifa.com:8000/aminazimi/aminpass@123/99101";
    const matchList = [];
    let mIds = [];
    for(let i =0; i < 10 ; i++)
    {
    ...
    const sqlMatches = `Select  hteam.team_name as hometeamname,awteam.team_name as awayteamname,hteam.img as homelogo,
    ...
    where ft_matchs.match_id in(`+mIds.join() +`) order by ft_match_time.match_datetime `;
    db.query(sqlMatches,(err,data) =>{
      if(err)
        ...
        res.render('index', { title: 'Live SportHD.com Broadcast',source:lv_url,'mlist':matchList });
       }
